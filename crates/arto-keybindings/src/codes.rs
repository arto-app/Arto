//! Physical key codes for shortcuts that the OS dispatches.
//!
//! Native menu accelerators and the JS interceptor's skip list both identify
//! a chord by its physical `Code` (e.g. `KeyW`) rather than the logical key,
//! because Alt/Option remaps `KeyboardEvent.key` on macOS while `event.code`
//! stays stable.

use keyboard_types::{Code, Key, Modifiers};

use crate::action::is_menu_action;
use crate::bindings::BindingSet;
use crate::shortcut::ShortcutSequence;

/// The modifiers and physical code of a single-chord shortcut.
///
/// This is what a native menu accelerator needs. Returns `None` for
/// multi-chord sequences (e.g. vim `g g`) and for keys with no physical
/// `Code` mapping; in those cases the caller keeps the cosmetic menu hint
/// and lets the keybinding engine handle the shortcut instead.
pub fn single_chord_code(key: &str) -> Option<(Modifiers, Code)> {
...
pub fn menu_accelerator_skip_key(key: &str) -> Option<String> {
    ...
    Some(format!("{}:{}", modifiers.bits(), code))
}
...
pub fn menu_accelerator_skip_keys(bindings: &BindingSet) -> Vec<String> {
    bindings
        .menu_shortcuts
        .iter()
        .filter(|ka| is_menu_action(&ka.action))
...
/// Map a logical `Key` to a physical `Code`, or `None` when unmappable.
...
mod tests {
    use super::*;
    use crate::bindings::KeyAction;

    /// Modifier a parsed `Cmd`/`Meta` chord resolves to on the host: `META`
    /// on macOS, `CONTROL` on Windows/Linux where the primary modifier is
    ...
    /// Modifier bits the primary accelerator carries in a skip key:
    ...
    fn cmd_letter_maps_to_primary_key_code() {
        assert_eq!(single_chord_code("Cmd+o"), Some((PRIMARY, Code::KeyO)));
    }

    #[test]
    fn cmd_shift_letter_includes_shift() {
        assert_eq!(
            single_chord_code("Cmd+Shift+o"),
            Some((PRIMARY | Modifiers::SHIFT, Code::KeyO))
        );
    }
    ...
    fn symbol_aliases_map_to_codes() {
        assert_eq!(
            single_chord_code("Cmd+BracketLeft"),
            Some((PRIMARY, Code::BracketLeft))
        );
        assert_eq!(single_chord_code("Cmd+Equal"), Some((PRIMARY, Code::Equal)));
        assert_eq!(single_chord_code("Cmd+Minus"), Some((PRIMARY, Code::Minus)));
    }

    #[test]
    fn digit_maps_to_digit_code() {
        assert_eq!(single_chord_code("Cmd+0"), Some((PRIMARY, Code::Digit0)));
    }

    #[test]
    fn no_modifier_named_key() {
        assert_eq!(
            single_chord_code("ArrowDown"),
            Some((Modifiers::empty(), Code::ArrowDown))
        );
    }

    #[test]
    fn multi_chord_sequence_has_no_single_code() {
        assert_eq!(single_chord_code("g g"), None);
    }

    #[test]
    fn invalid_key_returns_none() {
        assert_eq!(single_chord_code("no_such_key"), None);
        assert_eq!(single_chord_code(""), None);
    }

    // -- Skip-key canonical form (must match JS interceptor's event.code) --
    //
    // Modifiers bits: ALT=0x01, CONTROL=0x08, META=0x40, SHIFT=0x200.
    ...
    fn reserved_overrides_off_macos_include_emacs_ctrl_prefix_letters() {
        ...
        let overrides = reserved_key_overrides_impl(&crate::presets::emacs::bindings(), false);
    ...
    fn reserved_overrides_on_macos_exclude_ctrl_only_chords() {
        ...
        let overrides = reserved_key_overrides_impl(&crate::presets::emacs::bindings(), true);
        assert!(!overrides.contains(&"x".to_string()));
        assert!(!overrides.contains(&"v".to_string()));
        assert!(!overrides.contains(&"c".to_string()));
    }
    ...
    fn reserved_overrides_default_preset_leave_clipboard_reserved() {
        ...
        let bindings = crate::presets::default_bindings();
